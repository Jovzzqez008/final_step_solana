...
async def is_honeypot_goplus(client, token_address):
    if not GOPLUS_API_KEY: return False
    ...
    while True:
        try:
            ...
            for token_address, data in list(incubator.items()):
                is_honeypot = await is_honeypot_goplus(client, token_address)
                if is_honeypot is True:
                    logger.info(f"  - ❌ [HONEYPOT] {token_address[:10]} descartado por GoPlus.")
                ...
                if not dex_data:
                    if time.time() - data['found_at'] > 600:
                        logger.info(f"  - 🗑️ [DESCARTADO] {token_address[:10]} sin datos de DexScreener en 10 mins.")
                        del incubator[token_address]; await db_remove_from_incubator(token_address)
                    continue
                ...
                mensaje = (f"✅ **Token Aprobado**\n\n**Mint:** `{token_address}`\n**Liquidez:** `${liquidity:,.2f}`\n**Holders (aprox):** {holders}\n\n"
                           f"*(Chequeo Honeypot: {'OK' if is_honeypot is False else 'No disponible'})*\n"
                           f"*Recuerda verificar seguridad manualmente.*")
            ...
            for token_address, data in list(watchlist.items()):
                ...
                if status == 'approved' and age_hours > 4:
                    ...
                    if dex_data:
                        holders = dex_data.get('txns', {}).get('h24', {}).get('buys', 0)
                        if holders < 50:
                            logger.info(f"  - 🗑️ [DESCARTADO 4H] {token_address[:10]} no alcanzó 50 holders (tiene {holders}).")
                            mensaje = f"🗑️ **Token Descartado (4h)**\n\n**Mint:** `{token_address}`\nNo alcanzó los 50 holders."
                            if TARGET_CHAT_ID: await context.bot.send_message(chat_id=TARGET_CHAT_ID, text=mensaje, parse_mode='Markdown')
                            del watchlist[token_address]; await db_remove_from_watchlist(token_address)
                            continue
                        else:
                            logger.info(f"  - 👍 [SUPERVIVIENTE 4H] {token_address[:10]} superó el filtro con {holders} holders.")
                            watchlist[token_address]['status'] = 'survivor_4h'
                            await db_add_to_watchlist(token_address, watchlist[token_address])

                # NUEVO FILTRO DE 8 HORAS
                elif status == 'survivor_4h' and age_hours > 8:
                    logger.info(f"  - [CHECK 8H] Verificando crecimiento de {token_address[:10]}...")
                    dex_data = await get_dexscreener_data(client, token_address)
                    if dex_data:
                        holders = dex_data.get('txns', {}).get('h24', {}).get('buys', 0)
                        if holders < 250:
                            logger.info(f"  - 🗑️ [DESCARTADO 8H] {token_address[:10]} no alcanzó 250 holders (tiene {holders}).")
                            mensaje = f"🗑️ **Token Descartado (8h)**\n\n**Mint:** `{token_address}`\nNo alcanzó los 250 holders."
                            if TARGET_CHAT_ID: await context.bot.send_message(chat_id=TARGET_CHAT_ID, text=mensaje, parse_mode='Markdown')
                            del watchlist[token_address]; await db_remove_from_watchlist(token_address)
                            continue
                        else:
                            logger.info(f"  - 👍 [SUPERVIVIENTE 8H] {token_address[:10]} superó el filtro con {holders} holders.")
                            watchlist[token_address]['status'] = 'survivor_8h'
                            await db_add_to_watchlist(token_address, watchlist[token_address])
                
                # Reportes de 24, 72, 96h
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("👋 v10.4 (Watchlist Dinámica). Usa /cazar, /parar, /status.")
# ... (El resto de los comandos y main no cambia)
async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏹 ¡Iniciando caza con Watchlist Dinámica!")

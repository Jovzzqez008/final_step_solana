...
HELIUS_RPC_URL = os.getenv("HELIUS_RPC_URL") # Debe empezar con https://
HELIUS_WSS_URL = os.getenv("HELIUS_WSS_URL") # Debe empezar con wss://
...
async def processor_task(client):
    ...
    while True:
        try:
            ...
            await asyncio.sleep(3) # Pausa aumentada a 3 segundos
...
# --- ### NUEVO COMANDO PARA VER LA INCUBADORA ### ---
async def incubator_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = f"🐣 **Tokens en la Incubadora ({len(incubator)}):**\n\n"
    # Tomamos solo los primeros 10 para no hacer el mensaje demasiado largo
    for token_address in list(incubator.keys())[:10]:
        message += f"- `{token_address}`\n"
    
    if len(incubator) > 10:
        message += f"\n... y {len(incubator) - 10} más."
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v4.3 (Visor de Incubadora). Usa /cazar, /parar, /status, /incubadora.")
...
def main():
    ...
    # --- ### AÑADIMOS EL NUEVO MANEJADOR DE COMANDO ### ---

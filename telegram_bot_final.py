...
# --- ### FUNCIÓN DE PARSEO MANUAL MEJORADA Y MÁS ESTRICTA ### ---
def parse_manual_transaction(raw_tx_base64):
    try:
        ...
        for ix in msg.instructions:
            ...
            # Buscamos solo transacciones de Raydium
            if program_id == RAYDIUM_LP_V4_PROGRAM_ID:
                ix_data = ix.data
                # El "discriminador" de la instrucción 'initialize2' en Raydium es un byte específico.
                # Lo buscamos en los datos de la instrucción. Esto nos asegura que ES una creación de pool.
                is_initialize_ix = ix_data.startswith(b'\x01') # Discriminador para 'initialize2'
                
                if is_initialize_ix and len(ix.accounts) > 9:
                    mint_a = str(account_keys[ix.accounts[8]])
                    mint_b = str(account_keys[ix.accounts[9]])
...
async def processor_task(client):
    logger.info("Iniciando Procesador Manual Estricto...")
    while True:
        try:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v4.5 (Análisis Estricto). Usa /cazar, /parar, /status, /incubadora.")

async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get('tasks'): await update.message.reply_text("🤔 El bot ya está cazando."); return
    await update.message.reply_text("🏹 ¡Iniciando Cazador Paciente con Análisis Estricto!")

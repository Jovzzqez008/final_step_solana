...
async def incubator_checker_task(client, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:
        ...
        # --- BLOQUE DE CÓDIGO CORREGIDO ---
            ...
            logger.info("Vigilante de la Incubadora detenido.")
            break
...
async def watchlist_monitor_task(client, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:
        try:
            await asyncio.sleep(300) # Revisa la watchlist cada 5 minutos
            ...
            for token_address, data in list(watchlist.items()):
                ...
                if last_notified in notify_periods and age_hours >= notify_periods[last_notified]:
                    ...
                    if dex_data:
                        ...
                        logger.warning(f"  - No se pudieron obtener datos de DexScreener para el reporte de {token_address[:10]}.")
...
async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏹 ¡Iniciando la caza con motor de análisis y monitoreo de watchlist!")

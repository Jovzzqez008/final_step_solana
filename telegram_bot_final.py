...
MAX_VERIFICATION_ATTEMPTS = 3 # Número máximo de intentos de verificación
                ...
                for token_address, data in list(incubator.items()):
                    ...
                    if status == 'unverified':
                        ...
                        if attempts >= MAX_VERIFICATION_ATTEMPTS:
                            logger.info(f"  - [DESCARTADO] Token {token_address[:10]} descartado tras {attempts} intentos fallidos de verificación.")
                        ...
                        data['check_attempts'] = attempts + 1 # Incrementar siempre el contador

                        if reporte == "❓ Reporte de seguridad no disponible.":
                            logger.info(f"  - [ESPERANDO] GoPlus aún no tiene reporte para {token_address[:10]}.")
                            await db_update_incubator(token_address, data) # Guardar el nuevo número de intentos
                        elif es_seguro:
...
# ... (El resto del script, cazadores, watcher, comandos, etc. no cambia) ...
def enviar_alerta_telegram_sync(mensaje, chat_id):
    try:
        # Usamos requests aquí porque esta función no es async
        import requests
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v6.4 (Persistente)!\n\nUsa /cazar, /parar, /status, /diagnostico.")

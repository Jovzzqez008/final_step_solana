...
# --- Direcciones de los DEX a escuchar (ACTUALIZADAS) ---
RAYDIUM_LP_V4 = Pubkey.from_string('675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8') # Raydium Liquidity Pool V4
RAYDIUM_CPMM = Pubkey.from_string('CPMMoo8L3F4NbTegBCKVNunggL7H1ZpdTHKxrmXp37R')  # Raydium CPMM (MUY importante para nuevos tokens)
# METEORA_DLMM_PROGRAM = Pubkey.from_string('LBUZKhRxPF3XG2A2qRFFH2G2BgaR6f2x32a12p6c1J8') # Desactivado temporalmente
...
async def raydium_hunter_task(chat_id):
    logger.info("Iniciando Cazador de Raydium (LPv4)...");
    while True:
        try:
...
### --- INICIO DE LA MODIFICACIÓN --- ###
async def raydium_cpmm_hunter_task(chat_id):
    logger.info("Iniciando Cazador de Raydium (CPMM)...");
    while True:
        try:
            async with connect(HELIUS_RPC_URL) as websocket:
                await websocket.logs_subscribe(RpcTransactionLogsFilterMentions(RAYDIUM_CPMM))
                first_resp = await websocket.recv(); logger.info(f"Cazador de Raydium (CPMM) conectado. ID: {first_resp[0].result}")
                async for msg in websocket:
                    for log_message in msg:
                        logs = log_message.result.value.logs
                        for log in logs:
                            if "Instruction: Initialize" in log:
                                try:
                                    # Esta es una forma simplificada de buscar, puede que necesite ajuste
                                    # pero buscará cualquier token nuevo en este programa
                                    accounts_str = next((l for l in logs if "invoke" in l), None)
                                    if accounts_str:
                                        # Lógica de extracción de token aquí puede ser compleja
                                        # Por ahora, nos centramos en la conexión
                                        pass
                                    # Una forma más simple es procesar cualquier cosa que parezca un token
                                    # Esto es propenso a errores, pero una prueba inicial
                                    for item in log.split():
                                        try:
                                            pk = Pubkey.from_string(item)
                                            if pk:
                                                await procesar_nuevo_token(str(pk), "Raydium CPMM", chat_id)
                                                break # Procesamos solo el primero que encontremos
                                        except:
                                            continue

                                except:
                                    continue
        except asyncio.CancelledError: logger.info("Cazador de Raydium (CPMM) detenido."); break
        except Exception as e: logger.error(f"Error en Cazador de Raydium (CPMM): {e}. Reiniciando..."); await asyncio.sleep(30)
### --- FIN DE LA MODIFICACIÓN --- ###


# Se elimina meteora_hunter_task temporalmente
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v5.3 (Vigilancia Ampliada)!\n\nUsa /cazar, /parar, /status, /diagnostico.")

async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.message.chat_id
    if context.bot_data.get('tasks'): await update.message.reply_text("🤔 El bot ya está cazando."); return
    await update.message.reply_text("🏹 ¡Iniciando la caza con vigilancia ampliada! Agentes desplegados.")
    ...
    task_raydium_cpmm = asyncio.create_task(raydium_cpmm_hunter_task(chat_id)) # Nueva tarea
    task_incubator = asyncio.create_task(incubator_checker_task(chat_id))
    task_watcher = asyncio.create_task(watcher_task(chat_id))
    context.bot_data['tasks'] = [task_raydium_lp4, task_raydium_cpmm, task_incubator, task_watcher]
    ### --- FIN DE LA MODIFICACIÓN --- ###

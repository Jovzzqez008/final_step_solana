...
# --- Todas las funciones (setup_database, db_add_to_incubator, get_security_report, etc.) permanecen iguales ---
# ... (El código de las funciones no necesita ser mostrado de nuevo, solo la función main() cambia) ...
...
async def get_security_report(client, token_address):
...
async def procesar_nuevo_token(token_address, source, chat_id):
    ...
    if token_address in direcciones_a_ignorar: return
    ...
    if token_address not in incubator and token_address not in watchlist:
        ...
        new_data = {'found_at': time.time(), 'source': source, 'status': 'unverified', 'symbol': 'N/A', 'check_attempts': 0}
            ...
            try:
                ...
                for token_address, data in list(incubator.items()):
                    ...
                    if status == 'unverified':
                        attempts = data.get('check_attempts', 0)
                        if attempts >= MAX_VERIFICATION_ATTEMPTS:
                            logger.info(f"  - [DESCARTADO] Token {token_address[:10]} descartado tras {attempts} intentos fallidos.")
                            to_remove.append(token_address)
                            continue

                        logger.info(f"  - [VERIFICANDO] Chequeando seguridad de {token_address[:10]} (Intento {attempts + 1}/{MAX_VERIFICATION_ATTEMPTS})...")
                        reporte, es_seguro, _ = await get_security_report(client, token_address)
                        
                        data['check_attempts'] = attempts + 1
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v6.5 (Debug Key)!\n\nUsa /cazar, /parar, /status, /diagnostico.")
...
def main():
    ...
    # Imprimimos una porción de la clave para verificar que se está cargando correctamente
    if GOPLUS_API_KEY:
        logger.info(f"DEBUG: Cargando GOPLUS_API_KEY: {GOPLUS_API_KEY[:5]}...{GOPLUS_API_KEY[-5:]}")
    else:
        logger.error("DEBUG: ¡La variable de entorno GOPLUS_API_KEY no está configurada!")

...
# --- ### FUNCIÓN DE JÚPITER MODIFICADA PARA BUSCAR POR EDAD ### ---
async def get_jupiter_momentum_tokens(client):
    """Consulta DexScreener con criterios de momentum y edad."""
    # Buscamos tokens en Solana que tengan entre 12 y 24 horas de creados.
    query = "solana age > 12 hours and age < 24 hours"
    url = f"https://api.dexscreener.com/latest/dex/search?q={query}"
    try:
        res = await client.get(url, timeout=30)
        if res.status_code == 200 and res.json().get('pairs'):
            # Devolvemos solo los pares de Solana para seguridad
            return [p for p in res.json()['pairs'] if p.get('chainId') == 'solana']
    except Exception as e:
        logger.error(f"Error consultando DexScreener para Júpiter: {e}")
    return []
    ...
    while True:
        try:
            ...
            for token_address, data in list(incubator.items()):
                ...
                # El filtro de edad ya no es necesario aquí, ya que el Radar de Júpiter ya lo aplica.
                # Para los tokens del Cazador, se verificará solo la liquidez.
                if liquidity < 7500:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v9.0 (Júpiter Enfocado). Usa /cazar y /parar.")

async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get('tasks'): await update.message.reply_text("🤔 El bot ya está cazando."); return
    await update.message.reply_text("🏹 ¡Iniciando Doble Motor (Cazador de Raydium + Radar de Júpiter Enfocado)!")

...
# --- ### TAREA DE LA INCUBADORA CON PROCESAMIENTO SECUENCIAL ### ---
async def incubator_checker_task(client, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:
        try:
            await asyncio.sleep(5) # Pausa entre cada análisis de token
            if not incubator: 
                await asyncio.sleep(20) # Si no hay nada, espera un poco más
                continue
            
            # Tomamos el primer token disponible para analizarlo

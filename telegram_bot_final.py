...
async def incubator_checker_task(client, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:
        try:
            await asyncio.sleep(180) # Revisa la incubadora cada 3 minutos
            ...
            for token_address, data in list(incubator.items()):
                ...
                if not dex_data:
                    # Si no hay datos tras 1 hora, se descarta
                ...
                mensaje = (f"✅ **Alerta de Oportunidad**\n\n**Mint:** `{token_address}`\n\n"
                           ...
                           f"🚨 *Este token cumple tus criterios de edad y liquidez. Realiza tu análisis de seguridad manual en RugCheck.*")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("👋 v4.0 (Cazador Paciente). Usa /cazar y /parar.")

    ...
    try:
        ...
        for ix in msg.instructions:
            ...
            if str(program_id_pubkey) == RAYDIUM_LP_V4_PROGRAM_ID and len(ix.accounts) > 9:
                mint_a_pubkey = account_keys[ix.accounts[8]]
                mint_b_pubkey = account_keys[ix.accounts[9]]
...
async def get_dexscreener_data(client, token_address):
    ...
    try:
        ...
        if res.status_code == 200 and res.json().get('pairs'):
            main_pair = sorted(res.json()['pairs'], key=lambda x: x.get('liquidity', {}).get('usd', 0), reverse=True)[0]
            return main_pair
        return None
    except Exception as e: logger.error(f"Error consultando DexScreener para {token_address[:10]}: {e}"); return None
...
async def get_rugcheck_data(client, token_address):
    ...
    try:
        ...
        return None
    except Exception as e: logger.error(f"Error consultando RugCheck para {token_address[:10]}: {e}"); return None
...
# --- ### TAREA DEL PROCESADOR CON LÓGICA REAL ### ---
        ...
        try:
            ...
            if raw_tx:
                ...
                if token_address and (token_address not in incubator and token_address not in watchlist):
                    ...
                    incubator[token_address] = new_data; await db_add_to_incubator(token_address, new_data)
                else: logger.info(f"  - フィルター [FILTRADO] {signature[:10]}. Razón: {reason_parse}") # Filtro de calidad
            else: logger.info(f"  - フィルター [FILTRADO] {signature[:10]}. No se pudo obtener TX. Razón: {reason}")
            signature_queue.task_done()
            await asyncio.sleep(2) # Pausa para no saturar RPC
        ...
        try:
            ...
            for token_address, data in list(incubator.items()):
                ...
                if top_holders_ok:
                    ...
                    logger.info(f"  - ❌ [CENTRALIZADO] {token_address[:10]} descartado. Top holder: {holder_pct:.2f}%.")
                    del incubator[token_address]; await db_remove_from_incubator(token_address)

...
# VERSIÓN FINAL: Ajustado a las variables de entorno del usuario en Railway.
...
# Leemos las variables con los nombres EXACTOS que tienes configurados en Railway.

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

try:
    HELIUS_API_KEY = os.environ['HELIUS_API_KEY']
    # CORRECCIÓN: El script ahora busca 'HELIUS_RPC_URL' como en tu imagen.
    RPC_URL_WEBSOCKET = os.environ['HELIUS_RPC_URL']
    TELEGRAM_BOT_TOKEN = os.environ['TELEGRAM_BOT_TOKEN']
    TELEGRAM_CHAT_ID = os.environ['TELEGRAM_CHAT_ID']

    # Se cargan las otras variables que tienes para futuras expansiones (no se usan aún)
    BIRDEYE_API_KEY = os.environ.get('BIRDEYE_API_KEY')
    DATABASE_URL = os.environ.get('DATABASE_URL')

except KeyError as e:
    logging.error(f"Error: La variable de entorno {e} no está configurada en Railway. El bot no puede iniciar.")

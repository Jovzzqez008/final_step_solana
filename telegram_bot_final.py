...
# --- DIRECCIÓN ÚNICA Y MÁS IMPORTANTE ---
...
# --- FUNCIONES DEL BOT ---
# (Sin cambios hasta incubator_checker_task)
...
async def incubator_checker_task(chat_id):
    ...
    while True:
        try:
            ...
            promoted_tokens = []; expired_tokens = []; to_remove_unsafe = []
            
            current_time = time.time()
            
            # Usamos list() para iterar sobre una copia y poder modificar el original
            for token_address, data in list(incubator.items()):
                ...
                if status == 'unverified':
                    ...
                    if reporte_seguridad == "❓ Reporte de seguridad no disponible.":
                        logger.info(f"  - [ESPERANDO] GoPlus aún no tiene reporte para {token_address[:10]}. Se intentará en 5 mins.")
                        # No hacemos nada, simplemente lo reintentará en el próximo ciclo
                    
                    elif es_seguro:
                        logger.info(f"  - ✅ [OK SEGURIDAD] {token_address[:10]} es seguro. Pasando a vigilancia de mercado.")
                        data['status'] = 'verified'
                        data['security_report'] = reporte_seguridad
                        await db_update_incubator(token_address, data)
                    else:
                        logger.info(f"  - ❌ [RECHAZADO SEGURIDAD] {token_address[:10]} no es seguro. Razón: {reporte_seguridad}")
                        to_remove_unsafe.append(token_address)
                    
                    # AÑADIMOS UNA PAUSA PARA SER RESPETUOSOS CON LA API
                    await asyncio.sleep(1) # Pausa de 1 segundo entre cada chequeo de seguridad
                    continue # Pasamos al siguiente token sin chequear liquidez todavía

                # FASE 2: VIGILANCIA DE MERCADO (SOLO PARA TOKENS VERIFICADOS)
            ...
            # Limpieza al final del ciclo
            for token in promoted_tokens:
                if token in incubator: await db_remove_from_incubator(token); del incubator[token]
            for token in expired_tokens:
                if token in incubator: await db_remove_from_incubator(token); del incubator[token]; logger.info(f"  - 🚮 Token expirado y eliminado: {token[:10]}")
            for token in to_remove_unsafe:
                if token in incubator: await db_remove_from_incubator(token); del incubator[token]
...
# ... (El resto del script, raydium_hunter_task, watcher_task, comandos, etc., no cambia) ...
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v6.2 (Controlado)!\n\nUsa /cazar, /parar, /status, /diagnostico.")

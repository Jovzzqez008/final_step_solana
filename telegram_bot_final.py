...
def parse_manual_transaction(raw_tx_base64):
    try:
        ...
        # El "discriminador" de 8 bytes de la instrucción 'initialize2' de Raydium.
        # Esta es su "huella digital" única y la forma más precisa de identificarla.
        ...
        for ix in msg.instructions:
            ...
            if program_id == RAYDIUM_LP_V4_PROGRAM_ID:
                ix_data = ix.data
                is_initialize_ix = ix_data.startswith(INITIALIZE2_DISCRIMINATOR)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v4.7 (Verificación Precisa). Usa /cazar, /parar, /status, /incubadora.")

async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get('tasks'): await update.message.reply_text("🤔 El bot ya está cazando."); return
    await update.message.reply_text("🏹 ¡Iniciando Cazador Paciente con Verificación Precisa!")

            ...
            for token_address, data in list(incubator.items()):
                ...
                if status == 'unverified':
                    ...
                    if es_seguro:
                        ...
                        data['status'] = 'verified'; data['security_report'] = reporte_seguridad
...
# ... (El resto del código como raydium_hunter_task, watcher_task, etc. no cambia) ...
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v6.1 (Diagnóstico Paginado)!\n\nUsa /cazar, /parar, /status, /diagnostico.")
...
async def diagnostic_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mostramos un máximo de 20 para no superar el límite de Telegram
    limit = 20
    
    message = f"🐣 *Mostrando los primeros {min(total_tokens, limit)} de {total_tokens} tokens en la Incubadora:*\n\n"
    
    count = 0
    # Iteramos sobre una copia de los items para evitar problemas si el diccionario cambia

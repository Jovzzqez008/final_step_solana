...
DATABASE_URL = os.getenv("DATABASE_URL")  # <--- NUEVA VARIABLE DE ENTORNO
...
# --- 💾 NUEVAS FUNCIONES PARA LA BASE DE DATOS (POSTGRESQL) 💾 ---

async def setup_database():
    """Se conecta y asegura que las tablas 'incubator' y 'watchlist' existan."""
    try:
...
# --- Las funciones del bot ahora usan la DB ---
...
def get_security_report(token_address):
    ...
    try:
        ...
        if data.get('is_honeypot') == '1':
            report.append("- 🚨 ¡ALTO RIESGO! Posible Honeypot.")
            is_safe = False
        else:
            report.append("- ✅ No parece ser Honeypot.")
            
        total_lp_locked_pct = sum(float(lp.get('percent', 0)) for lp in data.get('lp_holders', []) if lp.get('is_locked') == 1)
        report_text = f"- 💧 Liquidez Bloqueada: {total_lp_locked_pct*100:.2f}%"
        report.append(report_text)
        
        if is_safe and data.get('is_honeypot') == '0':
            return "\n".join(report), True, report_text
        else:
            return "\n".join(report), False, report_text

    except Exception as e:
        logger.error(f"Error en GoPlus para {token_address}: {e}")
        return "❓ Error en GoPlus.", False, "N/A"

async def procesar_nuevo_token(token_address, source, chat_id):
    logger.info(f"Procesando nuevo token de '{source}': {token_address}")
    ...
    if es_seguro:
        if token_address not in incubator and token_address not in watchlist:
            logger.info(f"  - ✅ Pasa filtro de seguridad. Añadiendo a incubadora y DB: {token_address}")
            new_data = {'found_at': time.time(), 'source': source, 'security_report': reporte_seguridad}
            incubator[token_address] = new_data
            await db_add_to_incubator(token_address, new_data)
    else:
        logger.info(f"  - ❌ DESCARTADO (GoPlus): No pasó el filtro de seguridad. Razón: {reporte_seguridad}")
                        ...
                        for log in logs:
                            if "initialize2" in log:
                                try:
                                    ...
                                    if token_b: await procesar_nuevo_token(token_b, "Raydium", chat_id)
                                except: continue
                        ...
                        for log in logs:
                            if "Instruction: InitializePool" in log and "Program data: " in log:
                                try:
                                    ...
                                    if token_b: await procesar_nuevo_token(token_b, "Meteora", chat_id)
                                except: continue
...
async def watcher_task(chat_id):
    ...
    while True:
        try:
            ...
            logger.info("Vigía (Watcher) despertando...");
            current_time = time.time()
            survivors_to_check = []
            ...
            if survivors_to_check:
                ...
                for addr, data, hours in survivors_to_check:
                    ...
                    await db_update_watchlist_status(addr, watchlist[addr]['status'])
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get('tasks') and len(context.bot_data.get('tasks')) > 0:
        status_msg = (
            ...
            f"🐣 Hay **{len(incubator)}** tokens en la incubadora (memoria viva).\n"
            f"🕵️‍♂️ Hay **{len(watchlist)}** candidatos en la lista de vigilancia (memoria viva)."
        )

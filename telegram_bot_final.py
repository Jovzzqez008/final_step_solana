...
class Config:
    ...
    def _load(self):
        ...
        self.MAX_CONCURRENT_MONITORS = int(os.getenv('MAX_CONCURRENT_MONITORS', '500'))
        self.MAX_TOKENS_MONITORED = int(os.getenv('MAX_TOKENS_MONITORED', '500'))
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with self.session.post(url, json=payload, timeout=timeout) as resp:
                    ...
                    if resp.status != 200:
                        logging.debug(f"RPC {name} returned {resp.status}")
                        await self._rotate()
                        name, url = self._current_provider()
                        continue
                ...
                logging.debug(f"Timeout on {name}")
                await self._rotate()
                name, url = self._current_provider()
                continue
            except Exception as e:
                logging.debug(f"RPC error on {name}: {e}")
                await self._rotate()
                name, url = self._current_provider()
                continue
...
class StreamingManager:
    def __init__(self, cfg: Config, alert_engine: AlertEngine, db: Database, redis_client):
        ...
        self.running = False
        self.subscription_map: Dict[int, str] = {}
    ...
    async def connect(self):
        if not self.cfg.STREAMING_WSS:
            logging.warning("⚠️ No streaming WSS")
            return
    ...
    async def _subscribe_to_token(self, mint: str):
        ...
        try:
            ...
            logging.debug(f"📡 Subscribing to {token.symbol}")
        except Exception as e:
...
def create_app(bot_instance):
    app = FastAPI(title="Pump.fun Monitor v3.1", version="3.1.0")
    webhook_processed_mints = deque(maxlen=1000)

    @app.get("/")
    async def root():
        return {
            "status": "ok",
            "service": "pump.fun monitor v3.1",
            "version": "3.1.0",
...
async def handle_command(text: str, chat: Dict, bot_instance):
    ...
    if command == "/start":
        ...
        status_text = (
            ...
            f"🌐 RPC: *{rpc_provider}*\n\n"
            f"📈 *Estadísticas*\n"
            f"• Tokens: {len(streaming.token_data)}\n"
...
class PumpFunService:
    ...
    async def start(self):
        logging.info("🚀 Starting Pump.fun Monitor v3.1...")
        ...
        logging.info(f"✅ RPC ready: {rpc_provider}")
        ...
        if self.config.PUMPPORTAL_WSS:
            ...
            logging.info(f"✅ PumpPortal ready")
        ...
        logging.info("=" * 60)
        logging.info("📱 Bot NO se auto-inicia")
        logging.info("📱 Use /iniciar en Telegram")
        logging.info("=" * 60)
...
async def _main():
    ...
    logging.info("🎮 Bot ready - Esperando /iniciar")

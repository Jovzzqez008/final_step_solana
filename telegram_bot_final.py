...
watchlist = {}
incubator = {}
processed_signatures = set() # Para evitar procesar la misma transacción múltiples veces
...
async def procesar_nuevo_token(token_address, source, chat_id):
    # Ignorar direcciones conocidas (programas, SOL, USDC, etc.)
    if len(token_address) < 32 or len(token_address) > 44: return
    direcciones_a_ignorar = {'11111111111111111111111111111111', 'So11111111111111111111111111111111111111112', 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v'}
            ...
            async with connect(HELIUS_RPC_URL) as websocket:
                ...
                async for msg in websocket:
                    for log_message in msg:
                        ...
                        posibles_tokens = set()
                        for log in logs:
                            # Dividir cada línea de log en "palabras"
                            for word in log.split():
                                # Una heurística simple: las direcciones de token tienen entre 32 y 44 caracteres
                                if 32 <= len(word) <= 44:
                                    try:
                                        # Intentar validar si es una Pubkey válida
                                        Pubkey.from_string(word)
                                        posibles_tokens.add(word)
                                    except:
                                        # Si no es una dirección válida, simplemente la ignoramos
                                        continue
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 ¡Bienvenido al Bot Cazador PRO v5.7 (Extractor Universal)!\n\nUsa /cazar, /parar, /status, /diagnostico.")

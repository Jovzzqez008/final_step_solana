...
async def search_dexscreener(client):
    # Usamos la API de búsqueda para encontrar pares relevantes, no solo los más nuevos
    ...
    try:
        ...
        logger.error(f"Error buscando en DexScreener: {e}")
    return []
...
async def screener_task(client, context: ContextTypes.DEFAULT_TYPE):
    ...
    while True:
        try:
            ...
            if found_count == 0:
                logger.info(f"[EXPLORADOR] Se analizaron {len(candidate_pairs)} pares, pero ninguno cumplió los criterios.")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("👋 v12.5 (Búsqueda Inteligente). Usa /cazar, /parar, /status.")
async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔭 ¡Iniciando Búsqueda Inteligente de tokens!")

...
notified_tokens = set()
rejected_tokens = set() # <-- ### NUEVA LISTA NEGRA ###
    ...
    while True:
        try:
            ...
            for pair in pairs:
                ...
                # --- ### VERIFICACIÓN MEJORADA CON LISTA NEGRA ### ---
                if not token_address or token_address in notified_tokens or token_address in rejected_tokens:
                    continue
                ...
                if mint_renounced and top_holder_pct < holder_limit:
                    ...
                    logger.info(f"  - ❌ [RECHAZADO] {token_symbol} no pasa el filtro de seguridad. Añadido a lista negra.")
                    rejected_tokens.add(token_address) # <-- Añadimos a la lista negra
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v14.2 (Lista Negra). Usa /cazar, /parar, /status, /modo_estricto, /modo_flexible.")
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get('task'):
        status_msg = (f"✅ El bot está **Activo** (Modo: **{current_mode.upper()}**).\n\n"
                      ...
                      f"🚫 Tokens rechazados (memoria): **{len(rejected_tokens)}**")

...
async def get_birdeye_data(client, token_address):
    url = f"https://api.birdeye.so/defi/token_overview?address={token_address}"
    ...
    try:
        ...
        logger.error(f"Error consultando Birdeye para {token_address[:10]}: {e}")
        return None
...
async def processor_task(client):
    ...
    while True:
        try:
            ...
            # SIMULACIÓN: En un escenario real, aquí se llamaría a la función de parseo manual
...
async def incubator_checker_task(client, context: ContextTypes.DEFAULT_TYPE):
    logger.info("Iniciando Vigilante de la Incubadora (Birdeye + RugCheck)...")
    while True:
        try:
            ...
            for token_address, data in list(incubator.items()):
                ...
                # --- FILTRO 1: LIQUIDEZ (BIRDEYE) ---
                birdeye_data = await get_birdeye_data(client, token_address)
                
                if not birdeye_data:
                    if time.time() - data['found_at'] > 600:
                        logger.info(f"  - 🗑️ [DESCARTADO] {token_address[:10]} sin datos en Birdeye tras 10 mins.")
                ...
                if rugcheck_data:
                    ...
                    # --- ### LÍNEA MODIFICADA: LÍMITE AJUSTADO A 85% ### ---
...
async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏹 ¡Iniciando la caza con análisis combinado (Birdeye + RugCheck)!")

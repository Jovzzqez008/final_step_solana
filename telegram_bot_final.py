...
# --- PROGRAM IDs A VIGILAR ---
RAYDIUM_LP_V4_PROGRAM_ID = '675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8'
METEORA_VAULT_PROGRAM_ID = '24Uqj9JCLxUeoC3hGfh5W3s9FM9uCHDS2SG3LkddWBF'
...
def parse_manual_transaction(raw_tx_base64):
    try:
        ...
        for ix in msg.instructions:
            ...
            if program_id == RAYDIUM_LP_V4_PROGRAM_ID and len(ix.accounts) > 9:
            ...
            elif program_id == METEORA_VAULT_PROGRAM_ID and len(ix.accounts) > 4:
                mint_a = str(account_keys[ix.accounts[3]]); mint_b = str(account_keys[ix.accounts[4]])
                if mint_a not in known_mints: return mint_a, "Meteora"
                if mint_b not in known_mints: return mint_b, "Meteora"
    except: pass
    return None, None
...
# --- ### NUEVA FUNCIÓN PARA JÚPITER ### ---
async def get_jupiter_top_tokens(client):
    """Consulta la API de Júpiter para obtener los tokens con más transacciones."""
    url = "https://stats.jup.ag/coingecko/top_tokens" # Endpoint de tokens populares
    try:
        res = await client.get(url, timeout=30)
        if res.status_code == 200:
            return res.json()
    except Exception as e:
        logger.error(f"Error consultando Júpiter: {e}")
    return []


# --- TAREAS ASÍNCRONAS ---
async def hunter_task(program_id_str, source_name):
    logger.info(f"Iniciando Cazador para {source_name}..."); PROGRAM_PUBKEY = Pubkey.from_string(program_id_str)
    while True:
        try:
            async with connect(HELIUS_WSS_URL) as websocket:
                await websocket.logs_subscribe(RpcTransactionLogsFilterMentions(PROGRAM_PUBKEY)); await websocket.recv()
                logger.info(f"Cazador de {source_name} conectado.")
        ...
        except asyncio.CancelledError: logger.info(f"Cazador de {source_name} detenido."); break
        except Exception as e: logger.error(f"Error en Cazador de {source_name}: {e}. Reiniciando..."); await asyncio.sleep(15)
        ...
        try:
            ...
            if raw_tx:
                ...
                if token_address and (token_address not in incubator and token_address not in watchlist):
                    logger.info(f"  - ✅ [TOKEN ENCONTRADO en {source}] {token_address[:10]}... a la incubadora.")
    ...
    while True:
        try:
            ...
            for token in top_tokens:
                ...
                # Filtro de Momentum: Mucho volumen pero precio todavía bajo
                if token_address and (price > 0.00001 and price < 0.01):
                    if token_address not in incubator and token_address not in watchlist:
                        logger.info(f"  - 📡 [RADAR] Júpiter detectó {token.get('symbol', '')} ({token_address[:6]}...) con momentum. A la incubadora.")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 v6.0 (Multi-Fuente: Raydium+Meteora+Júpiter). Usa /cazar y /parar.")

async def hunt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get('tasks'): await update.message.reply_text("🤔 El bot ya está cazando."); return
    await update.message.reply_text("🏹 ¡Desplegando cazadores para Raydium, Meteora y el Radar de Júpiter!")
    ...
    context.bot_data.update({'client': client, 'tasks': [
        asyncio.create_task(hunter_task(RAYDIUM_LP_V4_PROGRAM_ID, "Raydium")),
        asyncio.create_task(hunter_task(METEORA_VAULT_PROGRAM_ID, "Meteora")),
        asyncio.create_task(jupiter_momentum_task(client)),
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get('tasks'):
        status_msg = (f"✅ El bot está **Activo** (Multi-Fuente).\n"
